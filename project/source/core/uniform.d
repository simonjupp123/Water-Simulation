...
class Uniform
{
    ...
    {
        assert(datatype == "vec2" || datatype == "vec3" || datatype == "vec4" || datatype == "mat4" || datatype == "Light", "unsupported uniform type");
    ...
    void Transfer()
    {
        ...
        else if (mDataType == "vec4")
        {
            vec4* v = cast(vec4*) mData;
            glUniform4f(mCachedUniformLocation, v.data[0], v.data[1], v.data[2], v.data[3]);
        }
        ...
        else if (mDataType == "Light")
        {
            //            Light* l = cast(Light*)mData;
            assert(0, "Not yet implemented");
            //            glUniform3f(mCachedUniformLocation,l.color[0],l.color[1],l.color[2]);
            //            glUniform3f(mCachedUniformLocation,l.position[0],l.position[1],l.position[2]);
        }
        else
        {
...
/// Light structure for uniform
struct Light
{
    vec3 color;
    vec3 position;
    this(vec3 color, vec3 position)
    {
        this.color = color;
