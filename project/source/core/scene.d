...
import linear, camera, scene, mesh, uniform, light, watermesh;
...
class SceneTree
{
	...
	void StartTraversal()
	{
		...
		foreach (child; mRootNode.mChildren)
		{
			...
			if (typeid(child) == typeid(WaterNode))
			{
				waterNodes ~= cast(WaterNode) child;
			}
		}
		...
		foreach (l; lights)
		{
			...
			l.mMaterial.mUniformMap["uModel"].Set(l.mModelMatrix.DataPtr());
			l.mMaterial.mUniformMap["uView"].Set(mCamera.mViewMatrix.DataPtr());
			l.mMaterial.mUniformMap["uProjection"].Set(mCamera.mProjectionMatrix.DataPtr());
			l.Update();
		}
		...
		foreach (m; meshes)
		{
			...
			if ("viewPos" in m.mMaterial.mUniformMap)
			{
				// Set the view position if it exists
				m.mMaterial.mUniformMap["viewPos"].Set(mCamera.mEyePosition.DataPtr());
			}
			if ("lightPos" in m.mMaterial.mUniformMap)
			{
				// Set the view position if it exists
				m.mMaterial.mUniformMap["lightPos"].Set(vec3(75.0f, 100.0, 75.0f).DataPtr());
			}

			m.Update();
		}
		foreach (w; waterNodes)
		{
			// Update all of the uniforms for the mesh.
			w.mMaterial.mUniformMap["uModel"].Set(w.mModelMatrix.DataPtr());
			w.mMaterial.mUniformMap["uView"].Set(mCamera.mViewMatrix.DataPtr());
			w.mMaterial.mUniformMap["uProjection"].Set(mCamera.mProjectionMatrix.DataPtr());
			w.mMaterial.mUniformMap["viewPos"].Set(mCamera.mEyePosition.DataPtr());
			w.mMaterial.mUniformMap["lightPos"].Set(vec3(75.0f, 100.0, 75.0f).DataPtr());
			w.Update();
		}
