...
class Pipeline
{
    ...
    /// Note: The pipeline path should be specified with a '/' at the end.
    ///       All shaders must otherwise be named 'vert.glsl' and 'frag.glsl' otherwise.
    /// TODO: Support for 'geo.glsl', 'tess.glsl', and 'mesh.glsl' will have to come in the future.
    /// TODO: Future 'pipelinePath' should likely just be a .json file with the paths otherwise specified
    ///       to make it even easier to setup configurations.
    this(string pipelineName, string pipelinePath)
    {
        assert(pipelinePath[$ - 1] == '/', "Last character for pipelinePath should be a '/'");
        string vertexShaderSourceFilename = pipelinePath ~ "vert.glsl";
        string fragmentShaderSourceFilename = pipelinePath ~ "frag.glsl";
        CompilePipeline(pipelineName, vertexShaderSourceFilename, fragmentShaderSourceFilename);
        writeln("Uniforms automatically parsed from: ", vertexShaderSourceFilename);
        writeln("\t", ParseUniforms(vertexShaderSourceFilename));
        writeln("Uniforms automatically parsed from: ", fragmentShaderSourceFilename);
        writeln("\t", ParseUniforms(fragmentShaderSourceFilename));
    }
    ...
    GLuint CompilePipeline(string pipelineName, string vertexShaderSourceFilename, string fragmentShaderSourceFilename)
    {
        ...
        void CheckShaderError(GLuint shaderObject)
        {
            ...
            if (result == GL_FALSE)
            {
                ...
                writeln("Error Messages Found:", errorMessages);
                assert(0, "Pipeline '" ~ pipelineName ~ "' failed to compile");
            }
        }
...
/// Investigates a shader and returns the uniforms found.
/// NOTE: This can be improved by fully parsing and populating 'structs' either through metaprogramming
///       by embedding shaders as D code, or otherwise parsing shader strings and building an in-memory struct.
//        The TODO list would be to call 'AddUniforms' automatically for each primitive and custom type."
string[] ParseUniforms(string filename)
{
    import std.file;
    ...
    if (exists(filename))
    {
        ...
        foreach (line; f.byLine)
        {
            ...
            {
                result ~= line.strip().split(" ").array.take(3).join(" ").dup;
            }
        }
    ...
    assert(0, "File " ~ filename ~ " does not exist when attempting to ParseUniforms");
}
...
// TODO: Write a basic linter for shader
//       Wishlist of checks
//       - [ ] one 'out' variable for fragment shader
//       - [ ] Make sure #version is consistent for fragment and vertex shader
//       - [ ] Check in/out pipeline for matches
//       - [ ] Identify heavilty nested code
// 			 - [ ] Make sure gl_Position is in vertex shader 
bool Linter(string vsource, string fsource)
{
    bool valid = true;
